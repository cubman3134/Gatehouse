 ...
 * Without this predicate the resume machinery in `browser.ts` has no caller at all.
 *
 ...
 * restart polls the same URL after it and sees the download resume, rather than a dead
 * `failed` it has no way to retry against — and the engine picks the surviving `.part` back up
 * through `createInterruptedDownload` instead of pulling the bytes again.
 *
 * That resume is conditional, and `planResume` is what decides: without an `eTag` or a
 * `Last-Modified` recorded from the original response there is no `If-Range` validator, so the
 * partial is discarded and the download starts over rather than risk a silently corrupt file.
 *
 ...
 * while the daemon was down is resumed rather than reclaimed out from under the download.
 *
...
export async function requeueInterrupted(
  ...
  for (const rec of store.all()) {
    ...
    // caller's re-POST onto this same job instead of starting a second download for the same
